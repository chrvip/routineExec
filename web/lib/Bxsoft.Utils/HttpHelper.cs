    ...
    public class HttpHelper
    {
        ...
        private HttpResult GetHttpRequestData(HttpItem objhttpitem)
        {
            ...
            catch (WebException ex)
            {
                ...
                if (response != null) {
                    ...
                    result.StatusDescription = "response 为 null";
                }
            }
            catch (Exception ex)
            {
        ...
        public static string GetHtml(string url)
        {
            ...
            HttpItem item = new HttpItem()
            {
                ...
                //Referer = "http://www.bxsoft.cn",//来源URL     可选项
                Allowautoredirect = true,//是否根据３０１跳转     可选项
                //CerPath = "d:\123.cer",//证书绝对路径     可选项不需要证书时可以不写这个参数
                //Connectionlimit = 1024,//最大连接数     可选项 默认为1024
                //Postdata = "1=2",//Post数据     可选项GET时不需要写
                //ProxyIp = "192.168.1.105",//代理服务器ID     可选项 不需要代理 时可以不设置这三个参数
                //ProxyPwd = "123456",//代理服务器密码     可选项
                //ProxyUserName = "administrator",//代理服务器账户名     可选项
                ResultType = DotNet.Utilities.ResultType.String//返回数据类型，是Byte还是String
