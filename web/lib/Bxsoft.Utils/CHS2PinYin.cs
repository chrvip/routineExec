...
namespace Bxsoft.Utils {
    ...
    public sealed class CHS2PinYin {
        ...
        private static SortedList<string, string> _Phrase;
        /**/
        /// <summary>
        /// 设置或获取包含列外词组读音的键/值对的组合。
        /// </summary>
        public static SortedList<string, string> Phrase {
            get {
                if (_Phrase == null) {
                    _Phrase = new SortedList<string, string>();
        ...
        public static string Convert(string chs, string separator, bool initialCap) {
            ...
            if (separator == null || separator.Length == 0)
                separator = "";
            // 例外词组
            foreach (var de in CHS2PinYin.Phrase) {
        ...
        private static char GetCharSpellCode(char c) {
            ...
            for (int i = 0; i < 26; i++) {
                if (areaCode[i] <= code && code <= (ushort)(areaCode[i + 1] - 1))
                    return (char)('A' + i);
            }
            return c;
        }
        private static SortedList<char, string> _Chars;
        /**/
        /// <summary>
        /// 设置或获取包含列外词组读音的键/值对的组合。
        /// </summary>
        public static SortedList<char, string> Chars {
            get {
                if (_Chars == null) {
                    _Chars = new SortedList<char, string>();
                    _Chars.Add('重', "Chong");
                    _Chars.Add('深', "Shen");
                    _Chars.Add('璐', "Lu");
                    _Chars.Add('旻', "Min");
        ...
        public static string GetPYSZ(string value) {
            string result = string.Empty;

            string aResult = string.Empty;
            foreach (char item in value.ToCharArray()) {
                if (Chars.TryGetValue(item, out aResult)) { // 例外字
                    result += aResult.Substring(0,1);
                }
                else {
                    result += GetCharSpellCode(item);
                }
            }
            return result;
